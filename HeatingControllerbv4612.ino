...
void sendMqttStatus(String topic, String data) {
   String topicc="stat/";
   topicc.concat(topic);
   char topicbuf[40], databuf[4];
  ...
  if(ui.keysBuf()) {
    ...
    switch (k+state*20) {
       ...
       case 4+state_prefs*20:
          ...
          ui.bl(bl);
          break;
       case 1+state_prefs*40:
       case 2+state_prefs*40:
       case 3+state_prefs*40:
       case 4+state_prefs*40:
          hmode=k-1;
          thmode= String(hmode);
          sendMqttStatus("heatcont/MODE",thmode);
          break;
...
//    Serial.println(humids[num]);
    ...
    parseDoorOpen(stopic,message);
  } else if (stopic.indexOf("cmnd/heatcont") == 0) {
    parseHeat(stopic,message);
  }
}

void parseHeat(String topic, String payload)
{
   hmode=payload.toInt();
}
...
int parseTopicNew(String topic) 
{
  ...
  String search = "tele/";
  int index = topic.indexOf(search)+search.length();
  String search_end = "/SENSOR";
  int index_end = topic.indexOf(search_end)+search_end.length();
  int num;
  if (index<search.length()) {
    num=0;
  } else {
...
int parsePayload(String payload, int select)
{
  ...
  int temp =0;
  if (payload.charAt(index+3) != ',') {
     temp=((payload.charAt(index)-'0')*100);
     temp=temp+((payload.charAt(index+1)-'0')*10);
     temp=temp+((payload.charAt(index+3)-'0'));
  } else {
     temp=((payload.charAt(index)-'0')*10);
     temp=temp+((payload.charAt(index+2)-'0'));
  }
  return temp;
}
...
//  while (!client.connected()) {
    ...
    if (client.connect(clientID)) {
      ...
      client.subscribe("cmnd/heatcont/MODE");
      client.subscribe("stat/heatcont/MODE");
      for (int i=0; i<sensors; i++) {
         char topicbuf[40];
